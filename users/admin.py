...
class CustomUserAdmin(UserAdmin):
    model = CustomUser
    list_display = ('document', 'document_type','person_type','first_name', 'last_name', 'email', 'is_staff','is_active','date_joined','last_login','display_groups','empresa',)  
    list_filter = ('is_staff','is_active','groups','document_type','person_type','empresa',)  
    search_fields = ('document', 'email', 'first_name', 'last_name','document_type','person_type','empresa',)
    ordering = ('document',)
    fieldsets = (
        (None, {
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            'fields': ('document', 'first_name', 'last_name','document_type','person_type', 'email', 'password1', 'password2')
        }),
    )
